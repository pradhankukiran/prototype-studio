...
class GeneratorTests(TestCase):
    ...
    def test_build_project_spec_contains_expected_shape(self):
        ...
        self.assertTrue(
            any(screen["screen_type"] == "dashboard" for screen in spec["screens"])
        )
    ...
    def test_build_project_spec_includes_rule_metadata(self):
        ...
        entity = EntityDefinition.objects.create(
            project=project, name="Quote", plural_name="Quotes", order=1
        )
        ...
        self.assertEqual(
            field_spec["calculation_expression"], "weekly_hours * hourly_rate"
        )
...
class BuilderWorkflowTests(TestCase):
    def setUp(self):
        ...
        self.entity_a = EntityDefinition.objects.create(
            project=self.project, name="Client", plural_name="Clients", order=1
        )
        self.entity_b = EntityDefinition.objects.create(
            project=self.project, name="Quote", plural_name="Quotes", order=2
        )
        ...
        self.state = WorkflowState.objects.create(
            project=self.project, name="Draft", is_initial=True, order=1
        )
    ...
    def test_entity_update_and_move(self):
        update_response = self.client.post(
            reverse(
                "entity-update",
                kwargs={"slug": self.project.slug, "entity_slug": self.entity_a.slug},
            ),
            {
                "name": "Participant",
                "plural_name": "Participants",
        ...
        move_response = self.client.post(
            reverse(
                ...
                kwargs={
                    ...
                    "entity_slug": self.entity_a.slug,
                    "direction": "down",
                },
            )
        )
    ...
    def test_field_delete_and_preview_page(self):
        preview_response = self.client.get(
            reverse("project-preview", kwargs={"slug": self.project.slug})
        )
    ...
    def test_invalid_field_submission_returns_inline_errors(self):
        ...
        self.assertContains(
            response, "Rule expression has invalid syntax.", status_code=400
        )
    ...
    def test_run_project_starts_runtime(self, mock_start_runtime):
        mock_start_runtime.return_value = {
            "pid": 99901,
            "port": 8751,
            "started_at": "2026-03-07T14:30:00+00:00",
        }
    ...
    def test_stop_project_stops_runtime(self, mock_stop_runtime):
        ...
        response = self.client.post(
            reverse("project-stop", kwargs={"slug": self.project.slug})
        )
