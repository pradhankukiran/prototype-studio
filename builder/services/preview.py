...
def _field_rule_tokens(field: dict) -> list[dict[str, str]]:
    ...
    if field.get("related_entity"):
        tokens.append(
            {"label": _title_from_slug(field["related_entity"]), "tone": "default"}
        )
    return tokens
...
def _build_list_preview(spec: dict, screen: dict) -> dict:
    entity = next(
        entity for entity in spec["entities"] if entity["slug"] == screen["entity"]
    )
    columns = ["ID"]
    ...
    for index, state_name in zip(range(1, 4), cycle(state_names), strict=False):
        ...
        for field in list_fields:
            row[field["label"]] = sample_context.get(
...
def _build_detail_preview(spec: dict, screen: dict) -> dict:
    ...
    return {
        "mode": "detail",
        "entity": entity,
        "workflow_state": spec["workflow_states"][0]["name"]
        if spec["workflow_states"]
        else None,
        "fields": [
            {
