...
def _wait_for_port(
    port: int, process: subprocess.Popen, timeout_seconds: float = 20.0
) -> None:
    deadline = time.monotonic() + timeout_seconds
    while time.monotonic() < deadline:
        if process.poll() is not None:
            raise PrototypeRuntimeError(
                "Streamlit exited before the preview became available."
            )
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
...
def start_project_runtime(project: PrototypeProject) -> dict:
    ...
    if not app_path.exists():
        raise PrototypeRuntimeError(
            "The generated Streamlit app is missing. Generate the package first."
        )
