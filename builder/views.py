...
from .forms import EntityForm, FieldForm, ProjectForm, ScreenDefinitionForm, WorkflowStateForm
from .models import EntityDefinition, FieldDefinition, GeneratedArtifact, PrototypeProject, ScreenDefinition, WorkflowState
...
from .services.runtime import PrototypeRuntimeError, get_project_runtime, start_project_runtime, stop_project_runtime
...
def _move_ordered_instance(instance, queryset, direction: str) -> bool:
    ...
    if direction == 'up':
        neighbor = siblings.filter(order__lt=instance.order).order_by('-order', '-pk').first()
    else:
        neighbor = siblings.filter(order__gt=instance.order).order_by('order', 'pk').first()
    if not neighbor:
        return False
...
def _build_rule_builder_config(entity: EntityDefinition, editing_field: FieldDefinition | None = None) -> dict:
    return {
        'fields': [
            {
                'name': field.name,
...
def _entity_detail_context(
    ...
    effective_edit_target = field_edit_target or (field_form.instance if field_form.instance.pk else None)
    return {
        'project': project,
        'entity': entity,
        ...
        'field_form_action': (
            reverse(
                'field-update',
                kwargs={'slug': project.slug, 'entity_slug': entity.slug, 'field_id': effective_edit_target.pk},
            )
            if effective_edit_target
            else reverse('field-add', kwargs={'slug': project.slug, 'entity_slug': entity.slug})
        ),
        'field_edit_target': effective_edit_target,
        'rule_builder_config': _build_rule_builder_config(entity, effective_edit_target),
    }
...
def _project_detail_context(
    ...
    *,
    ...
    entity_edit_target = (
        ...
        if request.GET.get('edit_entity') else None
    )
    state_edit_target = project.workflow_states.filter(pk=_parse_int(request.GET.get('edit_state'))).first()
    screen_edit_target = (
        project.screens.filter(pk=_parse_int(request.GET.get('edit_screen'))).select_related('entity').first()
    )

    if entity_form is None:
        entity_form = EntityForm(instance=entity_edit_target) if entity_edit_target else EntityForm()
    if state_form is None:
        state_form = WorkflowStateForm(instance=state_edit_target) if state_edit_target else WorkflowStateForm()
    ...
    return {
        ...
        'entity_form_action': (
            reverse('entity-update', kwargs={'slug': project.slug, 'entity_slug': entity_edit_target.slug})
        ...
        'state_form_action': (
            reverse('state-update', kwargs={'slug': project.slug, 'state_id': state_edit_target.pk})
        ...
        'screen_form_action': (
            reverse('screen-update', kwargs={'slug': project.slug, 'screen_id': screen_edit_target.pk})
            if screen_edit_target
...
def dashboard(request: HttpRequest) -> HttpResponse:
    projects = list(
        PrototypeProject.objects.filter(created_by=request.user).prefetch_related('entities', 'workflow_states', 'screens')
    )
    return render(
        request,
...
def project_detail(request: HttpRequest, slug: str) -> HttpResponse:
    project = get_project_for_user_with_prefetch(
        request, slug, prefetch_related=['entities__fields', 'workflow_states', 'screens__entity', 'artifacts'],
    )
    return render(request, 'builder/project_detail.html', _project_detail_context(project, request))
...
def entity_detail(request: HttpRequest, slug: str, entity_slug: str) -> HttpResponse:
    ...
    field_edit_target = entity.fields.filter(pk=_parse_int(request.GET.get('edit_field'))).first()
    field_form = (
    ...
    return render(request, 'builder/entity_detail.html', _entity_detail_context(project, entity, field_form, field_edit_target))
...
def add_entity(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        return redirect(project)
    messages.error(request, 'Could not add entity. Check the form values and try again.')
    return render(request, 'builder/project_detail.html', _project_detail_context(project, request, entity_form=form), status=400)
...
@login_required
def move_entity(request: HttpRequest, slug: str, entity_slug: str, direction: str) -> HttpResponse:
    project = get_project_for_user(request, slug)
    entity = get_object_or_404(EntityDefinition, project=project, slug=entity_slug)
    if request.method == 'POST':
        moved = _move_ordered_instance(entity, project.entities.all(), direction)
        messages.success(request, f'Moved {entity.name} {direction}.') if moved else messages.info(
            request,
...
def add_field(request: HttpRequest, slug: str, entity_slug: str) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        return redirect(entity)
    messages.error(request, 'Could not add field. Check the field type specific inputs.')
    return render(request, 'builder/entity_detail.html', _entity_detail_context(project, entity, form, None), status=400)


@login_required
def update_field(request: HttpRequest, slug: str, entity_slug: str, field_id: int) -> HttpResponse:
    project = get_project_for_user(request, slug)
    entity = get_object_or_404(
        EntityDefinition.objects.prefetch_related('fields', 'screens'),
        project=project,
    ...
    form = FieldForm(request.POST or None, instance=field, project=project, entity=entity)
    if form.is_valid():
        form.save()
    ...
    return render(request, 'builder/entity_detail.html', _entity_detail_context(project, entity, form, field), status=400)


@login_required
def delete_field(request: HttpRequest, slug: str, entity_slug: str, field_id: int) -> HttpResponse:
    project = get_project_for_user(request, slug)
    entity = get_object_or_404(EntityDefinition, project=project, slug=entity_slug)
    field = get_object_or_404(FieldDefinition, entity=entity, pk=field_id)
    if request.method == 'POST':
...
@login_required
def move_field(request: HttpRequest, slug: str, entity_slug: str, field_id: int, direction: str) -> HttpResponse:
    project = get_project_for_user(request, slug)
    entity = get_object_or_404(EntityDefinition, project=project, slug=entity_slug)
    field = get_object_or_404(FieldDefinition, entity=entity, pk=field_id)
    if request.method == 'POST':
        moved = _move_ordered_instance(field, entity.fields.all(), direction)
        messages.success(request, f'Moved {field.label} {direction}.') if moved else messages.info(
            request,
...
def add_workflow_state(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    return render(request, 'builder/project_detail.html', _project_detail_context(project, request, state_form=form), status=400)


@login_required
def update_workflow_state(request: HttpRequest, slug: str, state_id: int) -> HttpResponse:
    project = get_project_for_user_with_prefetch(
        request, slug, prefetch_related=['entities__fields', 'workflow_states', 'screens__entity', 'artifacts'],
    )
...
@login_required
def delete_workflow_state(request: HttpRequest, slug: str, state_id: int) -> HttpResponse:
    project = get_project_for_user(request, slug)
    state = get_object_or_404(WorkflowState, project=project, pk=state_id)
    if request.method == 'POST':
...
@login_required
def move_workflow_state(request: HttpRequest, slug: str, state_id: int, direction: str) -> HttpResponse:
    project = get_project_for_user(request, slug)
    state = get_object_or_404(WorkflowState, project=project, pk=state_id)
    if request.method == 'POST':
        moved = _move_ordered_instance(state, project.workflow_states.all(), direction)
        messages.success(request, f'Moved {state.name} {direction}.') if moved else messages.info(
            request,
...
def add_screen(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    return render(request, 'builder/project_detail.html', _project_detail_context(project, request, screen_form=form), status=400)
...
@login_required
def move_screen(request: HttpRequest, slug: str, screen_id: int, direction: str) -> HttpResponse:
    project = get_project_for_user(request, slug)
    screen = get_object_or_404(ScreenDefinition, project=project, pk=screen_id)
    if request.method == 'POST':
        moved = _move_ordered_instance(screen, project.screens.all(), direction)
        messages.success(request, f'Moved {screen.title} {direction}.') if moved else messages.info(
            request,
...
def run_project(request: HttpRequest, slug: str) -> HttpResponse:
    ...
    try:
        ...
        return redirect(project)
    messages.success(request, f'Prototype preview is running on port {runtime["port"]}.')
    return redirect(project)
...
def download_artifact(request: HttpRequest, artifact_id: int) -> FileResponse:
    ...
    return FileResponse(artifact_path.open('rb'), as_attachment=True, filename=Path(artifact.relative_path).name)
